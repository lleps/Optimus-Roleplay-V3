...
#include <a_samp>
#include "..\gamemodes\src\include\zcmd.inc"
...
CMD:flymode(playerid, params[]) {
		...
		if (GetPVarInt(playerid, "admin_level") < 2) return SendClientMessage(playerid, -1, "permisos insuficientes");
	    
		if (GetPVarInt(playerid, "ADMIN_ONDUTY") < 1) return SendClientMessage(playerid, -1, "No estás en servicio");
...
stock CancelFlyMode(playerid)
{
	...
	SetTimerEx("setPos2", 1000, "ifff", playerid, x, y, z);
	return 1;
}

forward setPos2(playerid, Float:x, Float:y, Float:z);
public setPos2(playerid, Float:x, Float:y, Float:z) {
